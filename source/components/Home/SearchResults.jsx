...
export class SearchResults extends Component {
  ...
  componentWillMount() {
    ...
    getResults( -1 ).then( function( response ) {
      this.setState( { pages: response.data.total_pages } );
    }.bind( this ) ).catch( function( error ) {
      console.log( error );
    } );
    // get the top ranked movies
    getResults( this.state.pages ).then( function( response ) {
      let results = [];
...
function getMatching( results, userInput ) {
  ...
  for ( let i = 0; i < results.length; i++ ) {
    if ( results[ i ].title.toLocaleLowerCase().includes( userInput.toLocaleLowerCase() ) ) {
